...
def printAllAdvertisesOf(username: str) -> None:
    if username != "":
        ...
        print("you need to enter a seller's name")
        print("try the command 'printallsellers' to see sellers' name")
...
def findAdvertBaseOnCriterias(transaction: str = "", location: str = "",
    ...
    if price != math.inf:
        if cmp == "equals":  # if cmp is '=', then we filter the advert based on the equals price given in parameters
            ...
            results = results.filter(Advert.price < price)
        elif cmp == "infequals":  # if cmp is '<=', then we filter the advert
            # based on the equals or lower price given in parameters
            results = results.filter(Advert.price <= price)
        elif cmp == "supequals":  # if cmp is '>=', then we filter the advert
    ...
    if userConnect["id"] is not None:
        try:
            ...
            if myadvert:  # if yes, then we can't make offer on our own advertises
                print("this advert is yours, you can't make offer on your own "
                      "advert !")
...
def printAllOffersIHaveMade() -> None:
    ...
    if userConnect["id"] is not None:
        # To print all advertises the connected user have,
        # we should make a query that related 3 tables (ProposeOffer, Offer, Advert):
        # make sure that it's the connected user who have made the offer and,
        # that the advert ,the offer is base on is not the connected user 's own
        # hence the filter
        ...
        if results: # if there is some results, we print them
        ...
        else: # unless, there is no offers from us
...
def printOffersOnMyAdvertOfId(id_advert: int = math.inf,
    ...
    if userConnect["id"] is not None:
        # to print all offers we had on an advert or we just have,we first check :
        try:
            if id_advert != math.inf:
                # if the id is valid, then we want to print offers for that advert only !
                # so we check if that advert existed and is owned by the user connected 's one
                request = session.query(Advert).filter(and_(Advert.id_advert ==
                                                            id_advert,
                                                            Advert.id_user ==
                                                            userConnect[
                                                                "id"])).one()
            else: # we try to find at least one advert that the connected user had
                request = session.query(Advert).filter(and_(
                    Advert.id_user ==
                    userConnect[
                        "id"])).one()
            # if we find some, we ckeck :
            # the advert is one the user connected owned
            # then, we check that the offer we are working on, has some offer
            # and finally, the owner of the advert is the user connected
            ...
            # if there is an advert we should filter the result on, we do it here
            if id_advert != math.inf:
                results = results.filter(Advert.id_advert == id_advert)

            if state_offer != "":  # if there is a state of offer(pounding, refused, or accpeted)
                # we should filter the result on, we do it here
                results = results.filter(Offer.state_offer == state_offer)

            results = results.all()

            if results:
                line_format = "| {:<10} | {:<20} | {:<10} | {:<10} | {:<15} | {:<15} | {:<10} | {:<10} | {:<30} |"
                header = line_format.format("UserID", "Username", "OfferID",
                                            "State", "Proposed Price",
...
def answerOfferOfId(id_offer: int, answer: str):
    ...
    if userConnect["id"] is not None:
        try:
            # first, we check if the offer with the id_offer existed
            offer = session.query(Offer).filter(
                Offer.id_offer == id_offer).one_or_none()
            try:
                # if yes, we check if the offer is the one of the user connected
                myOffer = session.query(ProposeOffer).filter(
                    and_(ProposeOffer.id_user != userConnect["id"],
                         ProposeOffer.id_offer == id_offer)).one()
                # if not, that means the offer hadn't been made by the user connected
                # and that is the correct process

                # so we check if the offer is in 'pounding' mode or not
            ...
            except NoResultFound: # the offer is yours (the user connected)
                print("this offer is yours, you can't answer your own offer !")
        except NoResultFound:
